...
public class ReportService {
    ...
    private ReportDTO convertToReportDTO(Report report, SimpleDateFormat sdf) {
        ...
        dto.setGeneratedByName(report.getGeneratedBy() != null ? report.getGeneratedBy().getName() : "---");
    ...
    public List<Map<String, Object>> getReports() {
        List<Object[]> rows = reportRepository.getAllReportsWithUser();
        ...
        for (Object[] row : rows) {
            ...
            map.put("templateName", row[2]);
            map.put("generatedByName", row[row.length - 1]); // 👈 last column
